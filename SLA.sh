...
if !	[ $# -ge 3 ]
	...
	then
		echo "Usage: ./SLA.sh [amount of ICMP packets to send] [round trip delay threshold value] [packet loss value]" >&2;
		echo "Example: sudo ./SLA.sh 1000 90 0.03" >&2;
		echo "It means: send 1k ICMP and ICMPv6 packets to the two defined anchor hosts, do the math and check the" >&2;
		echo "SLA assuming that the average RTD must be less than 100ms and that maximum PL must be 0.03%." >&2;
		echo "Recommended values are 10000 100 0.02. But be patient: probe can take more than one hour." >&2;
		exit 1;
...
if
	[[ "$PL" =~ ^[0-9]+$ && "$PP" -ge 1 && "$PP" -le 100 && "$PL" -ge 0 && "$PL" -le 100 ]] || \
	(( "$PP" >= 101 && "$PP" <= 1000 && (( $(echo "$PL >= 0.1" | bc -l) )) && (( $(echo "$PL <= 100" | bc -l) )) )) || \
	(( "$PP" >= 1001 && "$PP" <= 10000 && (( $(echo "$PL >= 0.01" | bc -l) )) && (( $(echo "$PL <= 100" | bc -l) )) )) || \
	(( "$PP" >= 10001 && "$PP" <= 100000 && (( $(echo "$PL >= 0.001" | bc -l) )) && (( $(echo "$PL <= 100" | bc -l) )) ))
...
#verify if we meet our v4 SLA
	if 	[[ ( "$RTD_v4_AVG" -gt "$RTD" ) || ( "$PL_v4_SUM" > "$PL" ) ]]
		...
		echo "During ping, packets can get lost. Here we count the percentage of the loss." >&2;
		echo "So, for ICMP between 1 and 100, PL value must be between 0 and 100" >&2;
		echo "For ICMP between 101 and 1000, PL value must be between 0.1 and 100" >&2;
		echo "For ICMP between 1001 and 10000, PL value must be between 0.01 and 100" >&2;
		echo "For ICMP between 10001 and 100000, PL value must be between 0.001 and 100" >&2;
		exit 1;
fi
